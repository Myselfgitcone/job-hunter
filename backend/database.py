...
from sqlalchemy import Column, String, Boolean, Integer, Text, DateTime, text, BigInteger, Float
...
# ── Encryption-at-rest for secret columns (API keys, bot token) ─────────────
# Keys were stored plaintext. EncryptedStr encrypts on write and decrypts on
# read transparently at the ORM layer, so no call-site changes are needed.
# Backward compatible: a value that isn't a valid ciphertext (legacy plaintext,
# or crypto unavailable) is returned unchanged, so nothing breaks during the
# gradual migration — old rows keep working and re-save encrypted.
try:
    from cryptography.fernet import Fernet
    _CRYPTO_OK = True
except Exception:  # pragma: no cover — falls back to plaintext if lib missing
    _CRYPTO_OK = False
...
def _fernet():
    """Cached Fernet from APP_ENCRYPTION_KEY, else derived from SECRET_KEY so it
    works without a new env var. NOTE: rotating SECRET_KEY without setting a
    stable APP_ENCRYPTION_KEY makes existing encrypted secrets unreadable — set
    APP_ENCRYPTION_KEY in prod to decouple the two."""
    global _FERNET
    ...
    if raw:
        try:
            ...
            pass
    secret = os.getenv("SECRET_KEY", "dev-secret-key-change-in-production-please")
    _FERNET = Fernet(base64.urlsafe_b64encode(hashlib.sha256(secret.encode()).digest()))
    return _FERNET


def decrypt_secret(value: str) -> str:
    """Decrypt a value read outside the ORM (e.g. raw SQL). Passthrough on any
    failure so legacy plaintext / already-decrypted values are returned as-is."""
    if not value or not _CRYPTO_OK:
        return value
    try:
...
class EncryptedStr(TypeDecorator):
    ...
    def process_bind_param(self, value, dialect):
        if not value or not _CRYPTO_OK:
            return value
        # Don't double-encrypt a value that's already a valid ciphertext.
        try:
...
# Pool sizing: the default (5 + 10 overflow = 15) is too small here — the
# scraper runs several groups concurrently while qualify/experience sweeps, the
# Telegram bot, the web app and the browser extension all hold connections.
# Exhausting it surfaced as "QueuePool limit of size 5 overflow 10 reached".
# 20 + 20 = 40 max, comfortably under a standard Postgres max_connections.
_pool_kwargs = {} if not _is_postgres else {
    "pool_size": 20,
    "max_overflow": 20,
    "pool_recycle": 1800,   # drop connections the provider may have already closed
    "pool_timeout": 30,
}
...
class Job(Base):
    ...
    indexed_at = Column(String, default="")   # FantasticJobs index time (finer than date-only posted_at)
...
class UserSettings(Base):
    ...
    ai_api_key           = Column(EncryptedStr, default="")   # OpenRouter key (legacy fallback)
    anthropic_api_key    = Column(EncryptedStr, default="")   # direct — console.anthropic.com
    google_api_key       = Column(EncryptedStr, default="")   # direct — aistudio.google.com
    openai_api_key       = Column(EncryptedStr, default="")   # direct — platform.openai.com
    ...
    # Telegram bot
    telegram_bot_token   = Column(EncryptedStr, default="")
...
class UserJob(Base):
    ...
    # Three-gate LLM scores from the tailoring run: ats/recruiter/hiring_manager
    # + overall + top_fixes. JSON object; None if scoring failed.
    gate_scores          = Column(Text, default=None)
    # What the analyze pass detected for this JD: target_cloud, company,
    # industry, role_domain, target_tools, present/missing. JSON object.
    ...
    # Per-resume AI cost: real tokens this tailor run consumed × model rate.
    tailor_cost          = Column(Float, default=None)   # USD for this tailoring run
    tailor_tokens_in     = Column(Integer, default=None)
    tailor_tokens_out    = Column(Integer, default=None)
