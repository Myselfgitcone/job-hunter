...
export const BASE: string = (import.meta.env.MODE === "development")
...
// Fetch an authenticated PDF endpoint as an object URL for inline <iframe>
// preview (an <iframe src> can't send the Authorization header). Caller must
// URL.revokeObjectURL() when done.
export async function fetchPdfBlobUrl(url: string): Promise<string> {
  const token = localStorage.getItem("jh_token");
...
export const api = {
  ...
  tailor: (id: string, signal?: AbortSignal, batch = false) => req<TailorResult>(`/api/jobs/${id}/tailor${batch ? "?batch=1" : ""}`, { method: "POST", signal }),
  ...
  baseResumePdfUrl: () => `${BASE}/api/profile/resume/pdf`,
  ...
  adminQualifyUsersBackfill: () => req<{ message: string }>("/api/admin/qualify-users-backfill", { method: "POST" }),
  getScrapeFamilies: () => req<Record<string, boolean>>("/api/admin/scrape-families"),
  setScrapeFamilies: (m: Record<string, boolean>) =>
    req<Record<string, boolean>>("/api/admin/scrape-families", { method: "PUT", body: JSON.stringify(m) }),
  ...
  getAnalytics: (personal?: boolean, userId?: string | null) =>
  ...
  updateJobMeta: (id: string, meta: { deadline?: string; interview_date?: string; priority?: number; deferred?: boolean }) =>
    req(`/api/jobs/${id}/meta`, { method: "PATCH", body: JSON.stringify(meta) }),
  setDeferred: (id: string, deferred: boolean) =>
    req(`/api/jobs/${id}/meta`, { method: "PATCH", body: JSON.stringify({ deferred }) }),
