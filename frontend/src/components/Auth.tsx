...
// BASE resolves to the Railway backend in prod (api.ts) — OAuth buttons must
// hit the backend, not the Vercel frontend origin.
...
export default function Auth({ onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    // Surface OAuth failures (e.g. "Google OAuth not configured", "Account not
    // found. Please register first.") instead of silently reloading the page.
    const err = params.get("error");
    if (err) { setError(err.replace(/\+/g, " ")); window.history.replaceState({}, "", "/"); }
  }, []);
          ...
          <div style={S.aside} className="auth-panel">
            ...
            <div style={{ marginTop: 36 }}>
              ...
              <div style={S.liveGrid}>
                {[
                  { value: liveStats?.last_scrape_mins_ago != null ? (liveStats.last_scrape_mins_ago < 60 ? `${liveStats.last_scrape_mins_ago}min ago` : `${Math.round(liveStats.last_scrape_mins_ago/60)}h ago`) : "—", label: "Last updated", color: "#64748b" },
