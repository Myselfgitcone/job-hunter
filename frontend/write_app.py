...
const SOURCES = ["All Sources","Greenhouse","Lever","Ashby","Workday","BambooHR","Recruitee","HiringCafe","Google","Apple","Meta","Netflix"];
const STATUS_ROWS = [
...
export default function App() {
  ...
  useEffect(() => {
    api.getSettings().then((s: any) => { if (s.last_scraped_at) setLastScraped(timeAgo(s.last_scraped_at)); }).catch(() => {});
  }, []);
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      setScrapeMsg("+" + r.new_jobs + " new"); setLastScraped("just now");
  ...
  const runAction = async (action: string) => {
    ...
    try {
      if (action === "qualify") { const r = await api.qualifyJob(selectedJob.id); updateJob(selectedJob.id, { qualify_result: r }); toast("Qualification complete", "success"); }
      else if (action === "resume") { const r = await api.tailor(selectedJob.id); updateJob(selectedJob.id, { tailored_resume: r.tailored_resume, ats_score_before: r.ats_before?.score ?? null, ats_score_after: r.ats_after?.score ?? null }); toast("Resume tailored", "success"); }
      else if (action === "fit") { const r = await api.tailor(selectedJob.id); updateJob(selectedJob.id, { fit_analysis: r.fit_analysis, interview_tips: r.interview_tips }); toast("Fit analysis ready", "success"); }
      else if (action === "cover") { const r = await api.generateCoverLetter(selectedJob.id); updateJob(selectedJob.id, { cover_letter: r.cover_letter }); toast("Cover letter generated", "success"); }
  ...
  const filtersActive = filters.posted !== "All" || filters.country !== "All Countries" || filters.locType !== "Any" || filters.source !== "All Sources" || filters.status !== "all";
  const navItems = [
    ...
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      <aside style={{ width: 208, flexShrink: 0, background: "var(--bg-surface)", borderRight: "1px solid var(--border-subtle)", display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        {view === "jobs" && (
          <div style={{ flex: 1, overflowY: "auto", padding: "0 14px 16px", borderTop: "1px solid var(--border-subtle)", marginTop: 2 }}>
            ...
            <div style={{ display: "flex", gap: 2, background: "var(--bg-base)", border: "1px solid var(--border-subtle)", borderRadius: 8, padding: 2 }}>
              {["All","24h","48h","7d"].map(o => <button key={o} onClick={() => setF("posted", o)} className={"seg-btn" + (filters.posted === o ? " active" : "")}>{o}</button>)}
            </div>
            ...
            {filtersActive && <button onClick={() => setFilters({ posted: "All", country: "All Countries", locType: "Any", source: "All Sources", status: "all" })} className="clear-btn" style={{ marginTop: 14, fontSize: 11, color: "var(--text-muted)", padding: "4px 6px", borderRadius: 6, transition: "all 120ms ease" }}>Clear filters</button>}
          </div>
        )}
        ...
        <div style={{ flexShrink: 0, padding: "10px 12px", borderTop: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", gap: 9 }}>
          <div style={{ width: 26, height: 26, borderRadius: 999, background: "linear-gradient(135deg,#8b5cf6,#6366f1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, color: "#fff", flexShrink: 0 }}>JR</div>
          <div style={{ lineHeight: 1.2, overflow: "hidden" }}>
            <div style={{ fontSize: 12, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Jagadish Reddy</div>
            <div style={{ fontSize: 10, color: "var(--text-muted)" }}>F1 / OPT</div>
          </div>
        </div>
      </aside>
      ...
      {view === "jobs" && viewMode === "kanban" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
          <Topbar scraping={scraping} scrapeMsg={scrapeMsg} lastScraped={lastScraped} onScrape={handleScrape} count={filteredJobs.length} viewMode={viewMode} setViewMode={setViewMode} IC={IC} />
...
with open(r'C:\Users\jagad\Downloads\job-hunter\frontend\src\App.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
